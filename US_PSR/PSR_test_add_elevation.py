...
########################### LOCAL ##################################
    OrderIDText = '427550'
    scratchfolder = r"E:\CC\luan\test1\test11"  # for regular .shp etc.
...
##        }
    ...
    for dsoid in dsoid_wells:
        ...
        arcpy.Clip_analysis(r"E:\CC\ErisSites.lyr", bufferSHP_wells, wells_clip)
    ...
    if int(arcpy.GetCount_management(os.path.join(wells_merge)).getOutput(0)) != 0:
        ...
        check_field = arcpy.ListFields(wells_sja,"Elevation")
        if len(check_field)==0:
            arcpy.AddField_management(wells_sja, "Elevation", "DOUBLE", "12", "6", "", "", "NULLABLE", "NON_REQUIRED", "")
        with arcpy.da.UpdateCursor(wells_sja,["Elevation"]) as uc:
            for row in uc:
                row[0]=-999
                uc.updateRow(row)
        del uc
        ...
        pntlist={}
        with arcpy.da.SearchCursor(wells_sja,["X","Y","ID"]) as uc:
            for row in uc:
        ...
        with arcpy.da.UpdateCursor(wells_sja,['Elevation','ID']) as uc:
            for row in uc:
                row[0]= result[row[1]]
                uc.updateRow(row)
        del row
