...
##    FIMpage.mergePage(page)
##
##    output = PdfFileWriter()
##    output.addPage(FIMpage)
##
...
##    t.drawOn(canvas,50,270)
    ...
    if NRF=='Y':
        ...
        canvas.line(50,180,PAGE_WIDTH-60,180)
        canvas.setFont('Helvetica-Bold', 12)
        canvas.drawString(70,160,"Please note that no information was found for your site or adjacent properties.")
##        Disclaimer=[]
##        p1 = Paragraph('<para alignment="justify"><font name=Helvetica size = 8>Please note that no information was found for your site or adjacent properties.</font></para>', style)
##        Disclaimer.append(p1)
##        Frame(110,70,PAGE_WIDTH,100).addFromList(Disclaimer,canvas)
    canvas.saveState()
##    canvas.restoreState()
##    p=None
##    Disclaimer = None
...
def selectbyyear(selectionLayer):
    ...
    # print "i = " + str(i)
    # print filepathList_b
    # print filenameList_b
    # print filepathList_m
    # print filenameList_m
...
def server_loc_config(configpath,environment):
    ...
    if environment == 'test':
        ...
        reportviewer_prod = configParser.get('server-config','reportcheck_prod')
...
reportcheckFolder = os.path.join(server_config['reportcheck'],'FIM')
...
shapefilepath = r'\\cabcvan1gis006\GISData\FIMS_USA\master\mastershps'
...
##    scratch = r'C:\Users\JLoucks\Documents\JL\test4'
    ...
    ##else:
    OrderIDText = '1129857'#arcpy.GetParameterAsText(0) #
    BufsizeText = '0.17'#arcpy.GetParameterAsText(1) #
    yesBoundary = 'yes'#arcpy.GetParameterAsText(2) #
    scratch =r'C:\Users\JLoucks\Documents\JL\fim2'#arcpy.env.scratchWorkspace
    emgOrder= 'N'
# -------------------------------------------------------------------------------------------------------------------------
    try:
...
##        if OrderType.lower() == "polyline" or OrderType.lower() == "polygon":
        ...
        arcpy.mapping.ExportToPDF(mxdFIP, FIPpdf, "PAGE_LAYOUT", 0, 0, 96, "BEST", "RGB", True, "ADAPTIVE", "RASTERIZE_BITMAP", False, True, "LAYERS_AND_ATTRIBUTES", True, 85)
        ...
        if ( yesBoundary.lower() == 'yes' and (OrderType.lower() == "polyline" or OrderType.lower() == "polygon")):
            ...
            if firstTime:
                ...
                arcpy.mapping.ExportToPDF(mxdFIP, shapePdf, "PAGE_LAYOUT", 0, 0, 96, "BEST", "RGB", True, "ADAPTIVE", "RASTERIZE_BITMAP", False, True, "LAYERS_AND_ATTRIBUTES", True, 85)
    ...
    if len(years_s) ==0 :
        ...
        summarydata = goSummaryPage(summaryfile,dictSummary_dedup,years_s)
        goCoverPage(coverfile)
...
# Save Summary ######################################################################################################
        ...
        try:
            ...
            v_return = 'Success'
            orc_return = cur.callfunc('eris_gis.addFimSummary', str, (str(summload),))
    ...
    if needViewer == 'Y':
        ...
        for year in years_s:
            ...
            arcpy.mapping.ExportToJPEG(mxd, os.path.join(scratch, viewerdir, imagename),df,3573,4000, color_mode='8-BIT_GRAYSCALE',world_file = True, jpeg_quality=50)
        ...
        if response_json['FIMUploadResult'] == 'OK':
            arcpy.AddMessage('xplorer upload: '+ str(response_json['FIMUploadResult']))
        else:
            arcpy.AddError('xplorer upload: '+ str(response_json['FIMUploadResult'])+ ' '+OrderNumText)
